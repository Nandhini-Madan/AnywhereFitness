...
router.post("/classes", (req, res) => {
  ...
  if (
    ...
    db.addClass(newClass).then((addedClass) => {
      res
        .status(201)
        .json({ data: addedClass, message: "successfully added class" });
    });
  }
});
                ...
                db.getClassById(id)
                  .then((updatedClass) => {
                    res
                      .status(200)
                      .json({
                        success: `successfully updated class with id ${id}`,
  ...
  db.getClassById(id)
    .then((returnedClass) => {
      if (returnedClass.instructor_id === instructorID) {
        db.removeClassById(id).then((count) => {
          if (count > 0) {
            res
              .status(200)
              .json({ message: `successfully deleted class with id ${id}` });
          } else {
            res
              .status(500)
              .json({ errMessage: `could not delete class with id ${id}` });
          }
        });
      } else {
        res
          .status(404)
          .json({
            message: `no class with id ${id} found `,
          });
      }
    })
    .catch((err) => {
